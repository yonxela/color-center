...
document.addEventListener('DOMContentLoaded', () => {
    console.log("Sistema Color Center: DOM cargado.");

    // Iniciar carga de datos de forma asíncrona (no bloqueante)
    if (supabase) {
        console.log("Conectando con Supabase...");
        loadFromSupabase().then(() => {
            console.log("Sincronización inicial completada.");
            // Refrescar vistas si es necesario
            if (state.currentView === 'control') control.init();
            if (state.currentView === 'dashboard') dashboard.init();
        }).catch(err => {
            console.error("Error en sincronización inicial:", err);
        });
    }

    // Ir a la pantalla de inicio por defecto
